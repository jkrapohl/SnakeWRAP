...
if [ "$pre_qc_report" = true ]; then
	...
	pqc1=${out}/pre-QC_report/${sample}_1_fastqc.html
	pqc2=${out}/pre-QC_report/${sample}_2_fastqc.html
	if [ ! -f "$pqc1" ] && [ ! -f "$pqc2" ]; then
		...
		echo "${sample}_1_fastqc.html and ${sample}_2_fastqc.html already exist. Skipping."
	fi
fi
...
if [ "$trim" = true ]; then
	...
	########################                 RUNNING TRIM-GALORE                    ########################
	########################################################################################################
	exists_test_1=${out}/${sample}_trimmed_1.fastq
	exists_test_2=${out}/${sample}_trimmed_2.fastq
	announcement "RUNNING TRIM-GALORE"
	if [ ! -f "$exists_test_1" ] && [ ! -f "$exists_test_2" ]; then
		trim_galore --no_report_file --dont_gzip --paired -o $out $reads_1 $reads_2
			
		# Fix the naming of the trimmed reads files:
		mv ${out}/${sample}_1_val_1.fq ${out}/${sample}_trimmed_1.fastq
		mv ${out}/${sample}_2_val_2.fq ${out}/${sample}_trimmed_2.fastq
		if [[ ! -s ${out}/${sample}_trimmed_1.fastq ]]; then error "Something went wrong with trimming the reads. Exiting."; fi
		...
		echo "${sample}_trimmed_1.fastq and ${sample}_trimmed_2.fastq already exist. Skipping."
	fi
fi
...
if [ "$bmtagger" = true ]; then
	...
	bmtagger_check_1=${out}/${sample}_final_pure_reads_1.fastq
	bmtagger_check_2=${out}/${sample}_final_pure_reads_2.fastq
	reads_1=${out}/${sample}_trimmed_1.fastq
	reads_2=${out}/${sample}_trimmed_2.fastq
	if [ ! -f "$bmtagger_check_1" ] && [ ! -f "$bmtagger_check_2" ]; then
		...
		${SOFT}/select_human_reads.py ${out}/${sample}.bmtagger.list $reads_1 > ${out}/human_reads/${sample}_host_reads_1.fastq
		${SOFT}/select_human_reads.py ${out}/${sample}.bmtagger.list $reads_2 > ${out}/human_reads/${sample}_host_reads_2.fastq
		...
		rm ${out}/${sample}_trimmed_1.fastq ${out}/${sample}_trimmed_2.fastq
		mv $reads_1 ${out}/${sample}_final_pure_reads_1.fastq
		mv $reads_2 ${out}/${sample}_final_pure_reads_2.fastq
		comm "Contamination-free and trimmed reads are stored in: ${out}/${sample}_final_pure_reads_1.fastq and ${out}/${sample}_final_pure_reads_2.fastq"
	else
		echo "${sample}_final_pure_reads_1.fastq and ${sample}_final_pure_reads_2.fastq alreadt exist Skipping"
	fi
fi	
...
if [ "$post_qc_report" = true ]; then
	...
	fastqc -t $threads -o ${out}/post-QC_report -f fastq ${out}/${sample}_final_pure_reads_1.fastq and ${out}/${sample}_final_pure_reads_2.fastq
